...
if(isset($_POST['btncheck'])){
  ...
  { 
    ...
    if(Module::IsPinValid($pin,$serial))
    {
      if(Module::IsPinUsed($pin,$serial))
      {
        if(Module::User($pin,$serial,$regNo))
        {
          ...
          header("location:presult.php?student=$student&session=$session&term=$term&class=$class");
        }
        else
        {          
          $msg="Scratch Card have been used by another student/student";
        }
      }
      else
      {  
        ...
        if(Finance::IsFee_Paid($student,$session,$term))
        {
          if(Module::UseCard($pin,$serial,"Used",$regNo,$session,$term))
            {
              $student=base64_encode($regNo);
              $session=base64_encode($session);
              $term=base64_encode($term);
              if($level=="Primary")
              {
                header("location:presult.php?student=$student&session=$session&term=$term");
              }
              elseif($level=="Secondary")
              {
                header("location:result.php?student=$student&session=$session&term=$term");
              }
            } 
        }
        else
        {
          $msg= "Please pay your $term School Fee and try again";
        }
    ...
    {
      $msg="Invalid Pin or Serial, Check and try again";
    }
  }
...
<script type="text/javascript">
  function loadsession(student,level)
  {
    ...
    xmlhttp.open("GET", "sessions.php?student="+student+"&level="+level, true);
    xmlhttp.send();
  }
</script>
</head>
            ...
            <div class="header_content d-flex flex-row align-items-center justify-content-start"  style="background: white">
              ...
              <nav class="main_nav_contaner ml-auto">
                <ul class="main_nav">
                  ...
                  <li class="active"><a href="../portal">Check Result</a></li>
      ...
      <div class="row">
        <div class="col">
          <div class="section_title_container text-center">
            <h2 class="section_title">Result Checker</h2>
          </div>
        </div>
      </div>
              ...
              <div>
                <form   method="POST" action="?" >
                  <table>
                    ...
                    <tr><td><label for="txtregno">Reg. No.</label></td><td colspan="2"><input type="text" name="txtregno" id="txtregno" required onkeyup="loadsession(document.getElementById('txtregno').value)" value="<?php echo $_GET['reg_no'];?>"  ></td></tr>
                    ...
                    <tr><td><label for="txtpin">PIN</label></td><td colspan="2"><input type="password" name="txtpin" id="txtpin" required></td></tr>
                    <tr><td><label for="txtserial">Serial</label></td><td colspan="2"><input type="text" name="txtserial" id="txtserial" required></td></tr>
                    <tr><td></td><td ><input type="reset" name="" value="Clear"></td><td><input type="submit" name="btncheck" id="btncheck" value="View Result"></td></tr>
                  </table>
                </form>
              </div>
            </div>
              ...
              <div class="news_post_meta">
                <p>
                  <ol>
                    ...
                    <li>Select Term</li>
                    <li>Enter Scratch Card Pin</li>
                    <li>Enter Scratch Card Serial</li>
