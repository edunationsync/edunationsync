...
<head> 
  ...
  <script type="text/javascript">
      ...
      {  
        ...
        function ProcessAllPositions(session,term,classs)
        {

          var subjects=[];

          subjects=JSON.parse('<?php echo json_encode($Subjectss); ?>');
          ...
          for (var i = 0; i <= subjects.length - 1; i++) {
            ...
            xmlhttp.onreadystatechange = function() {
              if (this.readyState == 4 && this.status == 200)
              {
                //UpdatePositions(subject,session,term);
                document.getElementById("entryMsg").innerHTML=this.responseText;
                //document.getElementById(subjects[i]+'position_status').innerHTML="Updated";
                document.getElementById("loader").innerHTML="Successfull";
                document.getElementById("preloader").style.display="none";
                //document.getElementById("msgContainer").innerHTML = this.responseText;
              }
            ...
            xmlhttp.open("GET", "ca_sheet/generate_subject_positions.php?subject="+subjects[i]+'&session='+session+'&term='+term+'&class='+classs , true);
            xmlhttp.send();
        ...
        function ClearPositions(subject,session,term,classs)
        {
          ...
          xmlhttp.open("GET", "ca_sheet/generate_subject_positions.php?subject="+subject+'&session='+session+'&operation=clear_position'+'&term='+term+'&class='+classs , true);
          xmlhttp.send();
        }
    ...
    <div id="content-wrapper">
      ...
      <div class="row"> 
        ...
        <div class="content" id="content" style="padding:20px 20px 20px 20px">
          ...
          <table width="100%">
            <thead><span id="class_position_status">Not Updated</span>
              <button onclick="ProcessAllPositions('<?php echo $Session; ?>','<?php echo $Term; ?>','<?php echo $Class; ?>')">GENERATE ALL SUBJECT NOW</button></thead>
                ...
                if(count($Subjectss)>0)
                {
                  foreach($Subjectss as $Subjects)
                  {
                    ...
                    <tr><td><a class="nav-link" href="ca_sheet/index.php?txtsessionp=<?php echo $Session; ?>&txttermp=<?php echo $Term ?>&txtclassp=<?php echo $Class; ?>&txtsubjectp=<?php echo $Subjects; ?>">
                        <i class="fas fa-fw fa-sheet"><img src="../images/closed-sheet.jpg" style="width: 20px"></i><span><?php echo $Subjects; ?></span></a></td><td>Status <?php if(Module::IsNotSubjectPositionUpdated($Session,$Term,$Class,$Subjects))
                          {
                            ?>
                            <span id="<?php echo $Subjects;?>position_status">Not Updated</span>
                            <button onclick="ProcessPositions('<?php echo $Subjects; ?>','<?php echo $Session; ?>','<?php echo $Term; ?>','<?php echo $Class; ?>')">GENERATE NOW</button>
                            <?php
                          }
                          else
                          {
                            ?><span id="<?php echo $Subjects;?>position_status">Updated</span>
                            <button onclick="ProcessPositions('<?php echo $Subjects; ?>','<?php echo $Session; ?>','<?php echo $Term; ?>','<?php echo $Class; ?>')">UPDATE NOW</button> <button onclick="ClearPositions('<?php echo $Subjects; ?>','<?php echo $Session; ?>','<?php echo $Term; ?>','<?php echo $Class; ?>')">CLEAR</button>
                            <?php
                          }
                          ?></td></tr>
