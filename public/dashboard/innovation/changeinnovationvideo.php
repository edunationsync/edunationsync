...
if(isset($_POST['btnUpdateInnovation']))
{
  ...
  if(Innovation::IsExist($innovation_id,$innovation_author_id,$innovation_title,$innovation_date))
  {
    ...
    if(is_uploaded_file($_FILES['innovation_video']['tmp_name'])){

      $innovation_video=base64_encode(file_get_contents($_FILES['innovation_video']['tmp_name']));
      if(Innovation::ChangeInnovationFile($innovation_id,$innovation_author_id,$innovation_title,$innovation_date,$innovation_video))
      {

        //Folder Upload Files Begins$filename=$_FILES['innovation_file']['name'];

        $filename=$_FILES['innovation_video']['name'];
        $ext=substr($filename, strpos($filename, '.'));
        $path = "../../files/videos/";
        $path = $path .$innovationdetails['innovation_author_id']." ".$innovation_id." ".$innovationdetails['innovation_title'].$ext;;
        if(move_uploaded_file($_FILES['innovation_video']['tmp_name'], $path)) 
        {      
          $file_name=$path;
        } 
        else
        ...
        $msg=$msg."<br/>Innovation Video Was Changed Successfully";
      }
      else{
        $msg=$msg."<br/>Innovation Video Upload Failed";
      }
    }
    else
    {
      $msg=$msg."<br/>Innovation Video was not uploaded";
    }
  }
  else
  {
...
<head> 
  ...
  <title>Update Innovation Video</title>
    ...
    <div class="card card-register mx-auto mt-5"> 
      <div style="padding: 20px 20px 20px 20px">
        <center><a href="../"   class="navi" ><i class="fa fa-home"></i> Dashboard</a> 
          ...
          <a href="changeinnovationfile.php?id=<?php echo $innovation_id; ?>" class="navi" ><i class="fa fa-home"></i>Change File</a></center>
      </div>


      <div class="card-header">Innovation Video Update</div>
            ...
            <div class="form-row"> 
              <div class="col-md-6">
                <div class="form-label-group">
                  ...
                  <input type="file" id="innovation_video" name="innovation_video" class="form-control" required="required">
                  <label for="innovation_file"><span style="color: red">(*)</span> Innovation Video</label>
                  <button type="submit" class="btn btn-primary btn-block" name="btnUpdateInnovation" id="btnUpdateInnovation" >Update Innovation Video</button>
                </div>
              </div>
            </div>
          </div>
